...
export async function getTxCount(address) {
  ...
  const url = `${base}/v2/monad/account/transactions?address=${encodeURIComponent(address)}&limit=1`
  ...
  // fallback to paginated counting
  let totalCount = 0, cursor = null, safety = 0
  do {
    const pageUrl = new URL(`${base}/v2/monad/account/transactions`)
